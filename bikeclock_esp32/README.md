...
| **GPIO 8** | 物理スイッチ (FUNC) | SW8 (FUNC) | 内部プルアップ使用 |
| **GPIO 13** | 物理スイッチ | SW3 | 内部プルアップ使用 |
| **GPIO 14** | 物理スイッチ | SW4 | 内部プルアップ使用 |
| **GPIO 35** | 物理スイッチ | SW5 | 内部プルアップ使用 |
| **GPIO 38** | 物理スイッチ | SW6 | 内部プルアップ使用 |
| **GPIO 39** | 物理スイッチ | SW7 | 内部プルアップ使用 |
...
> 空きGPIO: 0, 9, 15, 16, 17, 18, 21, 33, 34, 36, 37, 40, 41, 45-47（GPIO 0 は boot ピンなので入力用途では注意）
...
```
                            ...
                            ┌─────────────────────┐
                     USB ───┤ USB-CDC (書込/ログ)  │
                  (5V) ─────┼─→ 5V (VIN) / TM1637 VCC   3V3├──┬───── 3.3V ──→ ePaper VCC (3.3V)
                           │                              │
                   (GND) ──┴─→ GND                  GND├──┴───── GND  ──→ 全デバイス・全スイッチ共通GND
                           │                     │
                            │  GPIO4 (SW1)        │────────→ スイッチ SW1 (他端はGND)
                            │  GPIO5 (SW2)        │────────→ スイッチ SW2 (他端はGND)
                            │  GPIO13 (SW3)       │────────→ スイッチ SW3 (他端はGND)
                            │  GPIO14 (SW4)       │────────→ スイッチ SW4 (他端はGND)
                            │  GPIO35 (SW5)       │────────→ スイッチ SW5 (他端はGND)
                            │  GPIO38 (SW6)       │────────→ スイッチ SW6 (他端はGND)
                            │  GPIO39 (SW7)       │────────→ スイッチ SW7 (他端はGND)
                            │  GPIO8 (SW8 FUNC)   │────────→ スイッチ SW8/FUNC (他端はGND)
                           │                     │
...
| GPIO 4 | SW1 | 右矢印 (0x4F) | |
| GPIO 5 | SW2 | 下矢印 (0x51) | |
| GPIO 13 | SW3 | 上矢印 (0x52) | |
| GPIO 14 | SW4 | 左矢印 (0x50) | |
| GPIO 35 | SW5 | Enter (0x28) | |
| GPIO 38 | SW6 | Back (0x0224) | |
| GPIO 39 | SW7 | 再生/一時停止 (0xCD) | |
| GPIO 8 | **SW8 (FUNC)** | 表示モード切替（HIDキーではない） | 長押しでメンテナンスモード |
...
フェーズ別に段階開発中。現在 **Phase 4（設定永続化: LittleFS）まで完了**。
