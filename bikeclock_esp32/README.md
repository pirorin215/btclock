...
ESP32-S3 SuperMini で使用する全 GPIO を番号順に一覧します（電源ピンも併記）。各デバイスの配線イメージは後述の「配線図」を参照。

| GPIO | 用途 | 接続先 / 信号 | 備考 |
|:----:|------|--------------|------|
| 1 | ePaper | CS | SPI3_HOST 専用バス |
| 2 | ePaper | DC | SPI3_HOST 専用バス |
| 3 | ePaper | RST | ⚠ strapping pin（JTAG）。High-at-boot で運用・外付けプルアップ推奨 |
| 4 | BMI160 IMU | SDA | I2C（Phase 14-A: SW1 から明け渡し・再利用） |
| 5 | BMI160 IMU | SCL | I2C（Phase 14-A: SW2 から明け渡し・再利用） |
| 6 | TM1637 | DIO | |
| 7 | TM1637 | CLK | |
| 8 | スイッチ | SW8 (FUNC) | 内部プルアップ・長押しでメンテナンスモード |
| 10 | ePaper | BUSY | SPI3_HOST 専用バス |
| 11 | ePaper | MOSI (DIN) | SPI3_HOST 専用バス |
| 12 | ePaper | SCK (CLK) | SPI3_HOST 専用バス |
| 13 | スイッチ | SW3 上矢印 (0x52) | 内部プルアップ |
| 14 | スイッチ | SW4 左矢印 (0x50) | 内部プルアップ |
| 35 | スイッチ | SW5 Enter (0x28) | 内部プルアップ |
| 38 | スイッチ | SW6 Back (0x0224) | 内部プルアップ |
| 39 | スイッチ | SW7 再生/一時停止 (0xCD) | 内部プルアップ |
| 41 | スイッチ | SW1 右矢印 (0x4F) | 内部プルアップ ※Phase 14-A で GPIO4 から移動（背面端子） |
| 47 | スイッチ | SW2 下矢印 (0x51) | 内部プルアップ ※Phase 14-A で GPIO5 から移動（背面端子・露出確認要） |
| 48 | オンボード RGB LED | WS2812 | ボード内蔵・配線不要 |
| 3V3 | 3.3V 電源 | ePaper VCC | スイッチのプルアップは内部のため配線不要 |
| 5V (VIN) | 5V 電源 | TM1637 VCC | ePaper VCC (3.3V) は 3V3 ピンへ |
| GND | グラウンド | 全デバイス・全スイッチ共通 | |

> キー割り当て（0x4F 等）は BTClockMob アプリから変更可能（`SET:keys:` コマンド）。Phase 14-A で SW1/SW2 を GPIO 41/47（背面端子）へ移動し、空いた GPIO 4/5 を BMI160 IMU の I2C に再割当てしました。詳細は [TODO.md](../TODO.md)。

#### 使用禁止・非推奨ピン（新規割当時に回避）

| 区分 | GPIO | 理由 |
|------|:----:|------|
| **使用禁止**（ハードウェア占有） | **19 / 20** | USB D-/D+（USB-CDC 書込/ログに使用） |
| | **26〜32** | 内蔵フラッシュ/PSRAM に接続（ボード未露出） |
| **非推奨**（strapping pin） | **0** | SPI Boot / Download Boot 選択（Low で書込モード）。boot ボタン用途のみ |
| | **45** | フラッシュ電圧 VDD_SPI（High=1.8V で **3.3V フラッシュが確実にブート不全**）※最危険 |
| | **46** | ブートモード / ROM ログ出力。起動時レベルでブート挙動が変化 |
| | **3** | JTAG 信号源選択。※現在 ePaper RST で使用中（High-at-boot で運用・外付けプルアップ推奨） |

> strapping pin はリセット時にレベルがブート設定へ反映されるため、新規のスイッチ・I2C 等の入出力には **GPIO 0/3/45/46 を避け、汎用 GPIO（4/5/13/14/35/38/39/41/47 等）を使用**すること。
...
ESP32-S3            GY-BMI160
  ...
  3V3        ──── VCC
  GND        ──── GND
  (SW1/SW2 を GPIO 41/47 へ移動して空けた GPIO4/5 を使用)
                      SDO ──── GND   (I2C アドレス 0x68 固定)
```

> ⚠ GPIO4/5 は Phase 14-A で SW1/SW2 から**明け渡した**ピンです。SW1/SW2 は GPIO 41/47（背面端子）へ移動済み。**SW2=47 が背面端子で露出しているか配線時に確認**（未露出なら 42/40 等の別の非strappingピンへ。ピン定義は `bikeclock.h` の1箇所で切替）。
...
- **使用禁止・非推奨ピン**: 新規に GPIO を割り当てる場合は「GPIO 一覧（マスター）」の「使用禁止・非推奨ピン」（GPIO 0/3/19/20/26-32/45/46）を必ず確認すること。
