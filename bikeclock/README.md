...
BikeClockはXIAO BLEマイコンボードと4桁7セグメントLEDディスプレイを使用した、バイク搭載用のシンプルな時計デバイスです。

## 特徴

- ✅ **BLE時刻同期**: スマホアプリ（FastRecMob）で自動時刻補正
- ✅ **4桁LED表示**: TM1637使用の明るいディスプレイ（常時最大輝度）
- ✅ **シンプル接続**: 電源はバイクの12Vから5V変換のみ
- ✅ **常時駆動**: バイクから給電されている間は常に起動・表示
...
- **FastRec**: 録音レコーダーデバイス
- **FastRecMob**: Androidアプリ（BikeClockの制御にも使用）
