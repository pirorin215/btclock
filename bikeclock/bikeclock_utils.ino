    ...
    if (currentMillis - g_maintenanceState.lastInteractionMillis >= 3000) {
        ...
        switch (g_maintenanceState.currentMenu) {
            case MAINTENANCE_MENU_CANCEL:
                ...
                // Flash "1111" to distinguish from version display
                for (int i = 0; i < 3; i++) {
                ...
                logPrint("MAINTENANCE", "System rebooting...");
                Serial.flush();
...
void enterDfuMode() {
    ...
    logPrint("DFU", "Stopping BLE stack...");
    Serial.flush();
    ...
    logPrint("DFU", "Calling enterOTADfu()...");
    Serial.flush();
...
void startOtaDfuMode() {
    ...
    if (Bluefruit.connected()) {
        ...
        // This ensures the BLE stack has time to clean up
        logPrint("OTA", "Waiting for BLE disconnection...");
        unsigned long waitStart = millis();
        while (Bluefruit.connected() && (millis() - waitStart < 5000)) {
            delay(100);
        }
        logPrint("OTA", "BLE disconnected");
    }

    // Additional delay to ensure BLE stack is fully stopped
    logPrint("OTA", "Waiting for BLE stack to stabilize...");
    delay(2000);
