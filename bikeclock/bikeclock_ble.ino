...
// --- Adafruit OTA DFU ---
BLEDfu bledfu;  // Adafruit OTA DFU service
...
void onCommandWritten(uint16_t conn_hdl, BLECharacteristic* chr, uint8_t* data, uint16_t len) {
    ...
    if (len > 0 && len < 513) {
        ...
        } else if (strncmp(command, "START_OTA", 9) == 0 || strncmp(command, "DFU", 3) == 0) {
...
void setupBLE() {
    ...
    // IMPORTANT: Must be initialized first before other services
    logPrint("BIKECLOCK", "Initializing Adafruit OTA DFU Service...");
    bledfu.begin();
    ...
    Bluefruit.Advertising.setInterval(32, 244);    // 20ms to 152ms - Same as wip-broken-ota
    ...
    logPrint("BIKECLOCK", "Adafruit OTA DFU: ENABLED");
