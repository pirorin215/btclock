...
- ✅ **駐車検知（モーション認識）**: BMI160（6軸IMU）で駐車を検知し ePaper を詳細表示へ切替（キー抜き後の偽時刻残像を防止）。検出パターンを7セグに3秒表示
...
- ✅ **IMUデータ採取**: マイコンの6軸IMUを未来録り（10秒）で採取、ラベル付きCSVをダウンロードへ自動保存
- ✅ **モーション学習**: 採取データから特徴量を学習（重心法）し、モデルをマイコンへ送信（エッジ推論でマイコン単体がパターン検出）
